...
async function insertPendingUrl(url: string, title?: string): Promise<Response> {
  ...
  const { data, error } = await supabase
    .from("bookmarks")
    .insert({ url, title, type, status: "pending", user_id: OWNER_USER_ID })
    .select("id")
    .single();
...
async function insertReadyNote(text: string): Promise<Response> {
  const { data, error } = await supabase
    .from("bookmarks")
    .insert({
      url: null,
...
async function handleCapture(input: { url?: unknown; title?: unknown; text?: unknown }): Promise<Response> {
  ...
  if (url) {
    ...
    return insertPendingUrl(url, title);
  }

  if (text && isBareUrl(text)) return insertPendingUrl(text, title);
  if (text) return insertReadyNote(text);

  return json("capture requires url or text", 400);
}

async function handleRefresh(id: string): Promise<Response> {
