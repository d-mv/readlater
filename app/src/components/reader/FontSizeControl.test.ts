...
describe("FontSizeControl", () => {
  ...
  test("renders decrease and increase buttons", () => {
    ...
    expect(wrapper.find('[data-testid="font-size-decrease"]').exists()).toBe(true);
    expect(wrapper.find('[data-testid="font-size-increase"]').exists()).toBe(true);
  });

  test("clicking increase grows the article font size", async () => {
    const wrapper = mount(FontSizeControl);
    await wrapper.find('[data-testid="font-size-increase"]').trigger("click");
  ...
  test("clicking decrease shrinks the article font size", async () => {
    const wrapper = mount(FontSizeControl);
    await wrapper.find('[data-testid="font-size-decrease"]').trigger("click");
  ...
  test("disables decrease at the minimum font size", async () => {
    ...
    const wrapper = mount(FontSizeControl);
    expect(wrapper.find('[data-testid="font-size-decrease"]').attributes("disabled")).toBeDefined();
  });

  test("disables increase at the maximum font size", async () => {
    localStorage.setItem("articleFontSize", "24");
    const wrapper = mount(FontSizeControl);
    expect(wrapper.find('[data-testid="font-size-increase"]').attributes("disabled")).toBeDefined();
  });
});
